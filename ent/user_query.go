...
func (_q *UserQuery) QueryBorrowings() *BorrowingsQuery {
...
func (_q *UserQuery) WithBorrowings(opts ...func(*BorrowingsQuery)) *UserQuery {
	query := (&BorrowingsClient{config: _q.config}).Query()
...
func (_q *UserQuery) loadBorrowings(ctx context.Context, query *BorrowingsQuery, nodes []*User, init func(*User), assign func(*User, *Borrowings)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(borrowings.FieldUserID)
	}
	query.Where(predicate.Borrowings(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BorrowingsColumn), fks...))
	}))
